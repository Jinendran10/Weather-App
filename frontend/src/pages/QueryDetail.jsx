...
function RecordRow({ record, onDelete }) {
  return (
    <tr className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
      <td className="py-2.5 px-3 text-slate-700 font-mono text-sm">{record.record_date}</td>
      <td className="py-2.5 px-3 text-blue-600">{record.temp_min?.toFixed(1) ?? '—'}</td>
      <td className="py-2.5 px-3" style={{ color: 'var(--temp-accent)' }}>{record.temp_max?.toFixed(1) ?? '—'}</td>
      <td className="py-2.5 px-3 text-text-main font-medium">{record.temp_avg?.toFixed(1) ?? '—'}</td>
      <td className="py-2.5 px-3 text-text-sec">{record.humidity ?? '—'}%</td>
      <td className="py-2.5 px-3 text-text-sec">{record.wind_speed?.toFixed(1) ?? '—'}</td>
      <td className="py-2.5 px-3 text-text-sec max-w-xs truncate capitalize">{record.weather_description ?? '—'}</td>
  ...
  if (loadError) {
    return (
      <div className="card p-8 flex flex-col items-center gap-4 text-center border-red-200 bg-red-50">
        ...
        <p className="text-red-700 font-semibold">Failed to load query</p>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <button
            ...
            className="flex items-center gap-1 text-text-sec hover:text-primary text-sm mb-2 transition-colors"
          >
          ...
          <div className="flex items-center gap-4 mt-1.5 text-sm text-text-sec flex-wrap">
          ...
          {[
            ...
            <div key={s.label} className="stat-card">
              ...
              <p className="text-2xl font-bold text-text-main"
                 style={s.label.includes('Temp') ? { color: 'var(--temp-accent)' } : undefined}>
                {s.value}
              </p>
            </div>
          ))}
      ...
      {query.notes && (
        <div className="card p-4 flex items-start gap-3 border-amber-200 bg-amber-50">
          ...
          <p className="text-sm text-amber-800">{query.notes}</p>
        </div>
      )}
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                activeTab === t.id
                  ...
                  : 'text-slate-600 hover:text-text-main hover:bg-white'
              }`}
            >
          ...
          <div className="card p-5">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={chartData}>
                ...
                <Line type="monotone" dataKey="Min °C" stroke="#2563EB" strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="Avg °C" stroke="#64748b" strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="Max °C" stroke="#F97316" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
            ...
            <div className="card p-5">
              ...
              <ResponsiveContainer width="100%" height={180}>
                <BarChart data={chartData}>
                  ...
                  <Tooltip content={<CustomTooltip />} />
                  <Bar dataKey="Humidity" fill="#2563EB" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="card p-5">
              ...
              <ResponsiveContainer width="100%" height={180}>
                <BarChart data={chartData}>
                  ...
                  <Tooltip content={<CustomTooltip />} />
                  <Bar dataKey="Rain mm" fill="#38BDF8" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      {activeTab === 'table' && (
        <div className="card overflow-hidden">
          {sortedRecords.length === 0 ? (
            <div className="p-8 text-center text-slate-400">No records available.</div>
          ) : (
                ...
                <thead className="bg-slate-50 border-b border-slate-100">
                  <tr>
                    {['Date', 'Min °C', 'Max °C', 'Avg °C', 'Humidity', 'Wind m/s', 'Conditions', ''].map((h) => (
                      <th key={h} className="py-3 px-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-widest">
                        {h}
                      </th>
                    ))}
                  </tr>
